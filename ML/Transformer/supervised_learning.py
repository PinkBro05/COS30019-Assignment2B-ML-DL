...
def train_transformer(
    ...
    Note:
        - This function only trains the model and does not handle data loading and will be reused in the chunking approach.
    """
    model.to(device)
...
def train_with_chunking(args, device, data_collector, save_path, figures_dir):
    """Train the model using chunking approach for large datasets.
    ...
    try:
        ...
        encoders_scalers = first_chunk['encoders_scalers']
        ...
        return model, history, encoders_scalers
        
    except StopIteration:
        print("Error: No data chunks were generated. Please check your input file.")
        return None, None, None

def train_without_chunking(args, device, data_collector, save_path, figures_dir):
    """Train the model without chunking for smaller datasets.
    ...
    Returns:
        Trained model, training history, and encoders/scalers
    """
...
def test_model(args, model, device, test_loader, encoders_scalers, figures_dir):
    ...
    Args:
        ...
        encoders_scalers: Dictionary of encoders and scalers
        figures_dir: Directory to save test figures
...
def main():
    ...
    try:
        ...
        # Model, history, and encoders/scalers variables
        model = None
        encoders_scalers = None
        test_loader = None
        
        # Check whether to test or train
        if args.test:
            print("Testing mode selected. Loading model and preparing test data...")
            ...
            # Load model architecture based on whether chunking is enabled
            if args.use_chunking:
                # Initialize encoders and get model metadata from a chunk
                data_collector._initialize_encoders_on_full_dataset(args.data_file)
                chunk_generator = data_collector.process_data_in_chunks(
                    input_file=args.data_file,
                    chunk_size=args.chunk_size,
                    initialize_encoders=False
                )
                ...
                # Create model architecture (will be filled with weights from saved file)
                model = TransformerModel(
                    input_dim=input_dim,
                    d_model=args.d_model,
                    num_heads=args.num_heads,
                ...
                test_loader = test_loaders['val_loader']
            else:
                # Non-chunking approach for testing
                data_loaders = data_collector.get_data_loaders(
                    data_file=args.data_file,
                    batch_size=args.batch_size,
                    shuffle=False,
            ...
            # Test the model
            test_metrics = test_model(args, model, device, test_loader, encoders_scalers, figures_dir)
        else:
            print("Training mode selected. Starting training process...")
            # Training phase - check if using chunking or regular approach
            if args.use_chunking:
                # Train the model using chunking approach
                model, history, encoders_scalers = train_with_chunking(args, device, data_collector, save_path, figures_dir)
            else:
                # Train the model without chunking
                model, history, encoders_scalers = train_without_chunking(args, device, data_collector, save_path, figures_dir)
        ...
        print(f"For testing: python Transformer/supervised_learning.py --data_file {os.path.join(data_path, '_sample_final_time_series.csv')} --test --model_path Transformer/save_models/transformer_traffic_model.pth")
