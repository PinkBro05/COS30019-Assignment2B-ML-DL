...
# Add data_reader directory to path
data_reader_dir = os.path.join(parent_dir, "data_reader")
sys.path.append(data_reader_dir)

try:
    # Try importing directly from data_reader package
    from data_reader.parser import parse_graph_file
except ImportError:
    try:
        # Try importing from parser module directly
        sys.path.append(os.path.join(parent_dir, "data_reader"))
...
try:
    ...
    try:
        from entity.DfsNetwork import DfsNetwork
    except ImportError:
        print("Could not import DfsNetwork class. Check paths and module availability.")
        sys.exit(1)
