...
class TrafficDataTransformer:
    ...
    def read_data_files(self, year):
        ...
        dfs = []
        for file in sorted(csv_files)[:5]:  # Limit to first 5 files for sample creation
            try:
                df = pd.read_csv(file)
                dfs.append(df)
    ...
    def read_all_data_files(self, year):
        """Read all VSDATA files for a specific year (no limit).
    ...
    def create_sample(self):
        """Create a sample of transformed data from the first few days of 2014."""
        ...
        raw_data = self.read_data_files(sample_year)
        ...
        if raw_data is not None:
            ...
            if sample_transformed is not None:
                ...
                sample_file = os.path.join(self.output_path, 'sample_trans.csv')
    ...
    def process_all_years(self):
        ...
        for year in self.years:
            ...
            raw_data = self.read_all_data_files(year)
...
def main():
    ...
    data_path = os.path.join(base_path, 'Raw', 'main')
    
    # Years to process (modify as needed)
    ...
    # Ask user if they want to proceed with processing all data
    proceed = input("Sample data created. Proceed with processing all years? (y/n): ")
    ...
    if proceed.lower() == 'y':
        ...
        if processed_years:
            print(f"Successfully transformed and saved data for years: {', '.join(processed_years)}")
            print(f"Each year's data was saved to a separate CSV file in: {transformer.output_path}")
    else:
        print("Skipping full data processing. Sample data is available for review.")
